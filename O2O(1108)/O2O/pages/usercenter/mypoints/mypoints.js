...
Page({
  ...
  loadMore(type) {
    ...
    app.fetch("user/pointsDetail.htm", { fromIndex: this.data.fromIndex, type: type, limit: 10 }).then(res => {
  ...
  onLoad: function (options) {
    ...
    let task = app.fetch("user/points/his.htm", {}).then(res => {
      return res.data.data;
    });
    let detail = app.fetch("user/pointsDetail.htm", { fromIndex: this.data.fromIndex, type: this.data.tagList[this.data.activeIndex].type, limit: 10 }).then(res => {
      return res.data.data;
  ...
  changeTag(e) {
    ...
    app.fetch("user/pointsDetail.htm", { fromIndex: this.data.fromIndex, type: this.data.tagList[this.data.activeIndex].type, limit: 10 }).then(res => {
      wx.hideLoading();
      if (res.data.success) {
        this.setData({
