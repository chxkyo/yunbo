...
Page({
  ...
  onLoad: function (options) {
    ...
    app.fetch("/snail-portal/product/productList.htm", { unitId: app.globalData.unitId }).then(res => {
      if (res.data.success) {
          wx.hideLoading();
          this.setData({
      ...
      app.fetch("/snail-portal/product/productList.htm", { unitId: app.globalData.unitId, categoryId: categoryList[0].id }).then(res=>{
        wx.hideLoading();
  ...
  changeCategoryIndex(e){
    ...
    app.fetch("/snail-portal/product/productList.htm", { unitId: app.globalData.unitId, categoryId: id }).then(res => {
      wx.hideLoading();
      res.data.data.list.forEach(function (val, index) {
        val.productImgPath = app.globalData.baseUrl + val.productImgPath;
      })
